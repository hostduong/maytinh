...
func XuLyGuiOTPEmail(c *gin.Context) {
	...
	if !ok { c.JSON(200, gin.H{"status": "ok", "msg": "Nếu đúng Email, mã đã được gửi!"}); return }
	...
	c.JSON(200, gin.H{"status": "ok", "msg": "Mã xác minh đã gửi vào Email!"})
}
...
func XuLyQuenPassBangOTP(c *gin.Context) {
	...
	if !ok || !nghiep_vu.KiemTraOTP(kh.TenDangNhap, otp) {
		c.JSON(200, gin.H{"status": "error", "msg": "Mã OTP không đúng!"}); return
	}
